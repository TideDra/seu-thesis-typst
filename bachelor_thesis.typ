#import "template.typ": acknowledgment, appendix, bachelor_conf, seu_bibliography
...
#let info = (
  title: [基于四十二号混凝土的意大利面配料方案研究],//手动换行请传入列表，一个元素为一行:("基于四十二号混凝土的意大利面配料方","案研究")
  student_id: [12345678],
  name: [张三],
  college: [霍格沃兹学院],
  major: [母猪产后护理],
  ...
  en_abstract: [
    英文摘要应与中文摘相对应，250 个实词左右。采用第三人称介绍该学位论文内容，叙述的基本时态为一般现在时，确实需要强调过去的事情或者已经完成的行为才使用过去时、完成时等其他时态。
...
// 对以下文本应用模板
#show: doc => bachelor_conf(doc, ..info)
...
插图力求精炼，且每个插图均应有图序和图名。图序与图名位于插图下方，图序默认按章节编排，如@fig:rain_dist（#ref(<chap:chap2>)第1个图），在插图较少时可以全文连续编序，如图10。设置 ```typ bachelor_conf(continuous_index: true, ...)``` 启用连续编序。

如一个插图由两个及以上的分图组成，分图用(a)、(b)、(c)等标出，并标出分图名，如@subfig:v_set。

简单文字图可用Typst直接绘制（如CeTZ包），复杂的图考虑使用相应的图形绘制软件完成，以提高图形表达质量~@sanh2021multitask。目前Typst不支持插入PDF图像，矢量图请转为SVG格式。
...
#figure(
  table(
    ...
    table.hline(),
    [],
    table.cell(colspan: 4, [Stage 1 (>7.1 μm)]),
    table.cell(colspan: 4, [Stage 2 (4.8-7.1 μm)]),
    table.cell(colspan: 4, [Stage 3 (3.2-4.7 μm)]),
    table.hline(),
...
公式及文字中的一般变量（或一般函数）（如坐标 $X$、$Y$，电压 $V$，频率 $f$）宜用斜体，矢量用粗斜体如 $bold(S)$（```typ $bold(S)$```）或白斜体上加单箭头 $arrow(S)$（```typ $arrow(S)$```），常用函数（如三角函数 $cos$、对数函数 $ln$ 等）、数字运算符、化学元素符号及分子式、单位符号、产品代号、人名地名的外文字母等用正体。

公式排版时使用 ```typ $ equation $```，在 `eqaution` 和 `$` 之间留一个空格进入单行公式模式。公式中插入 `&` 作为对齐符，使用 `\` 换行。

$ V_"cell" = E_"OCV" - V_"act" - V_"ohm" - V_"conc" $ <eq:1>
$
  E_"OCV" = & 1.229 - 0.85 times 10^(-3) (T_(s t) - T_0) \
...
注释采用小五号宋体，英文及数字为小五号 Times New Roman 字体，利用 ```typ #footnote[]``` 插入。
