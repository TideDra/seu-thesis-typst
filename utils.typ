...
#let get_chapter_idx_display(loc: none) = locate(cur_loc => {
  ...
  let is_appendix = if part_state.at(loc) == "appendix" {
    true
  } else {
    false
  }
...
#let set_doc_basic(doc) = {
  ...
  // 粗体样式。
  show strong: it => text(font: 字体.宋体, weight: "bold")[#it]
...
#let set_doc_figure(doc, continuous_index: false) = {
  ...
  show ref: it => {
    ...
    if el != none and el.func() == figure and el.kind == "subfigure" {
      ...
      locate(loc => {
        // 获得所在大图
        let parent_fig = query(figure.where(kind: image).before(subfig_loc), loc).last()
        // 获得大图序号
        let parent_fig_idx = parent_fig.counter.at(subfig_loc).first()
        // 获得章节序号
        let chapter_idx = get_chapter_idx_display(loc: subfig_loc)
        // 子图引用格式:图2-1(a)
        return link(subfig_loc)[图#{chapter_idx}-#parent_fig_idx~(#it)]
      })
    } else {
      if el != none and el.func() == figure {
        ...
        return link(
          ...
          locate(loc => {
            ...
            if continuous_index {
              // 图表引用格式同caption格式
              str(figure_numbering(..el.counter.at(el.location()), kind: el.kind, loc: el.location()))
            } else {
